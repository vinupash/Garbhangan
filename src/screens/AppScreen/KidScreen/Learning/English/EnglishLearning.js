...
const EnglishLearning = ({ navigation }) => {
    ...
    const EducationTab = ({ onPress, title }) => {
        return (
            <View style={[styles.boxContent, { width: windowWidth >= 1280 ? '47%' : '40%', }]}>
                <TouchableOpacity
                    activeOpacity={0.98}
                    style={[styles.sectionBtn, { minHeight: windowWidth >= 1280 ? 80 : 60, }]}
                    onPress={onPress}
                >
                    <ImageBackground
                        source={assets.WoodBord}
                        style={{ width: '100%', height: windowWidth >= 1280 ? 150 : 85, resizeMode: 'contain', position: 'relative', justifyContent: 'center', alignItems: 'center', ...SHADOWS.light }}
                    >
                        <Text style={{ fontFamily: FONT.Charlatan, fontSize: windowWidth >= 1280 ? SIZES.xxl : SIZES.mediumLarge, color: '#FFFFFF', letterSpacing: 1 }}>{title}</Text>
                    </ImageBackground>
                </TouchableOpacity>
            </View>
        )
                ...
                <View style={{ flexDirection: 'column', justifyContent: 'space-between', height: '100%', alignItems: 'center' }}>
                    ...
                    >
                        <View style={styles.cardSection}>
                            <EducationTab
                                title="Rhymes"
                                onPress={() => navigation.navigate('EnglishLearningStack', { screen: 'EnglishRhymes' })}
                            />

                            {/* <EducationTab
                                title="Story"
                                onPress={() => navigation.navigate('EnglishLearningStack', { screen: 'EnglishStory' })}
                            /> */}

                            <EducationTab
                                title="Alphabet"
                                onPress={() => navigation.navigate('EnglishLearningStack', { screen: 'EnglishAlphabet' })}
                            />

                            <EducationTab
                                title="Math's"
                                onPress={() => navigation.navigate('EnglishLearningStack', { screen: 'EnglishMaths' })}
                            />

                            <EducationTab
                                title="General Science"
                                onPress={() => navigation.navigate('EnglishLearningStack', { screen: 'EnglishGeneral' })}
                            />
                        </View>
                    </ScrollView>
                </View>
