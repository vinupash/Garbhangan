...
const KidsEducation = ({ navigation }) => {
    ...
    const EducationTab = ({ onPress, title }) => {
        return (
            <View style={[styles.boxContent, { width: windowWidth >= 1280 ? '47%' : '38%', }]}>
                <TouchableOpacity
                    activeOpacity={0.98}
                    style={[styles.sectionBtn, { minHeight: windowWidth >= 1280 ? 80 : 80, }]}
                    onPress={onPress}
                >
                    <ImageBackground
                        source={assets.WoodBord}
                        style={{ width: '100%', height: windowWidth >= 1280 ? 100 : 80, resizeMode: 'contain', position: 'relative', justifyContent: 'center', alignItems: 'center', ...SHADOWS.light }}
                    >
                        <Text style={{ fontFamily: FONT.Charlatan, fontSize: windowWidth >= 1280 ? SIZES.xl : SIZES.mediumLarge, color: '#FFFFFF', letterSpacing: 1 }}>{title}</Text>
                    </ImageBackground>
                </TouchableOpacity>
                ...
                <View style={{ flexDirection: 'column', justifyContent: 'space-between', height: '100%', alignItems: 'center' }}>
                    ...
                    >
                        <View style={styles.cardSection}>
                            <EducationTab
                                title="English"
                                onPress={() => navigation.navigate('EnglishLearningStack', { screen: 'EnglishLearning' })}
                            />

                            <EducationTab
                                title="मराठी"
                                onPress={() => navigation.navigate('MarathiLearningStack', { screen: 'MarathiLearning' })}
                            />

                            <EducationTab
                                title="हिंदी"
                                onPress={() => navigation.navigate('HindiLearningStack', { screen: 'HindiLearning' })}
                            />

                            <EducationTab
                                title="Math's"
                                onPress={() => navigation.navigate('MathsLearningStack', { screen: 'MathsLearning' })}
                            />

                            <EducationTab
                                title="General Science"
                                onPress={() => navigation.navigate('GeneralScienceLearningStack', { screen: 'GeneralScienceLearning' })}
                            />
                        </View>
                    </ScrollView>
                </View>
...
const styles = StyleSheet.create({
    ...
    cardSection: {
        ...
        flexWrap: "wrap",
        // justifyContent: 'space-between',
        width: windowWidth >= 1280 ? windowWidth - 200 : windowWidth - 100,
        alignSelf: 'center',
